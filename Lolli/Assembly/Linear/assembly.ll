...
asm_evalPG(N, P, PC, K) <-    
    ...
    bang(get_nth_command(P, PC, C)) x
    asm_evalC(C, PC, NewPC, asm_evalPG(M, P, NewPC, K)).
%! asm_evalPG(+Gas, +Program, +ProgramCounter, +Continuation).

asm_interp(P, Vars, S) <-
    stack([]) -> asm_evalP(P, 0, stack(_) x asm_collect(Vars, S)).
%! asm_interp(+Prog, +Vars, -State).

asm_interpG(N, P, Vars, S) <-
    stack([]) -> asm_evalPG(N, P, 0, stack(_) x asm_collect(Vars, S)).
%! asm_interpG(+Prog, +Vars, -State).
