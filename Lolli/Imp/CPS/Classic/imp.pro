...
collect([variable(X, V) | XS1], [(X, V) | XS2]) <-     
    collect(XS1, XS2).
%! collect(+StateOutCeval, -StateOut).

%--- INTERP ---
interp(StateIn, Prog, StateOut) <-
    ceval(StateIn, Prog, collect(StateOutCeval, StateOut), StateOutCeval).
%! interp(+StateIn, +Prog, -StateOut).

%--- INTERPG ---
interpG(N, StateIn, Prog, StateOut) <-
    cevalG(N, StateIn, Prog, collect(StateOutCeval, StateOut), StateOutCeval).
%! interpG(+Gas, +StateIn, +Prog, -StateOut).
