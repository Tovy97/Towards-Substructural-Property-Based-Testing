...
testP(N) :- 
    ...
    query(N, P, F1),
    sort(F1, F),    
    init_var(State, _, Type),
    prove(styping(State, Type)),
    ( 
        (prove(com_type(P, Type)), prove(interp(State, P, S1)), sort(S1, S), F == S) -> 
            write(" passato\n\n") ; 
            write(" non passato\n\n")
    ),
    write("---PROGRAMMA---\n"),
...
testP_SS(N) :- 
    ...
    prove(styping(State, Type)),
    ( 
        (prove(com_type(P, Type)), prove(interpSS(State, P, S1)), sort(S1, S), F == S) -> 
            write(" passato\n\n") ; 
            write(" non passato\n\n")
    ),
    write("---PROGRAMMA---\n"),
...
testN(N) :- 
    ...
    (
        (E == loop) ->
        (
            (prove(interpG(G, State, P, S))) -> 
            write(" non passato\n\n") ; 
            write(" passato\n\n")
        ) ; (
            (E == noVar) ->
            (
                (prove(interp(State, P, S))) -> 
                write(" non passato\n\n") ; 
                write(" passato\n\n")
            ) ;
            (
...
testN_SS(N) :- 
    ...
    (
        (E == loop) ->
        (
            (prove(interpSSG(G, State, P, S))) -> 
            write(" non passato\n\n") ; 
            write(" passato\n\n")
        ) ; (
            (E == noVar) ->
            (
                (prove(interpSS(State, P, S))) -> 
                write(" non passato\n\n") ; 
                write(" passato\n\n")
            ) ;
            (
