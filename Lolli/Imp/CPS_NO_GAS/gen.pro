...
get_N_test(prog, C, S, _, Vars) :- 
    ...
    (
        (N == 0) -> (
            ...
            nb_getval(tests, Gs),
            nb_setval(tests, [P | Gs]), 
            fail
        )
    ).
get_N_test(exp, C, S, T, Vars) :- 
    ...
    (
        (N == 0) -> (
            ...
            nb_setval(num_test, M),
            nb_getval(tests, Gs),
            nb_setval(tests, [(T,E) | Gs]), 
            fail
        )
    ).
get_N_test(_, _ ,_, _, _).
%! get_N_test(+WhatGenerate, +Certificate, +Dimension, +Type, +Vars).

getTest(W, C, N, S, T, G, Vars) :-
    nb_setval(num_test, N),
    nb_setval(tests, []),
    get_N_test(W, C, S, T, Vars),
    nb_getval(tests, G),
    nb_delete(num_test),
    nb_delete(tests).
%! getTest(+WhatGenerate, +Certificate, +NumberOfTest, +TypeOfExpression, -GeneratedTests, +Vars).
