...
testP(N) :- 
    ...
    ( 
        (seq(CertSeq, Gamma, [], com_type(P)), seq(CertSeq, [], Delta, interp(P, Vars, S1)), sort(S1, S), F == S) -> 
            write(" passato\n\n") ; 
            write(" non passato\n\n")
    ),
    write("---PROGRAMMA---\n"),
...
testP_SS(N) :- 
    ...
    seq(CertSeq, Gamma, Delta, styping(Vars)),
    ( 
        (seq(CertSeq, Gamma, [], com_type(P)), seq(CertSeq, [], Delta, interpSS(P, Vars, S1)), sort(S1, S), F == S) -> 
            write(" passato\n\n") ; 
            write(" non passato\n\n")
    ),
    write("---PROGRAMMA---\n"),
...
testN(N) :- 
    ...
    (
        (E == loop) ->
        (
            (seq(CertSeq, [], Delta, interp(P, Vars, S))) -> 
            write(" non passato\n\n") ; 
            write(" passato\n\n")
        ) ; (
            (E == noVar) ->
...
testN_SS(N) :- 
    ...
    (
        (E == loop) ->
        (
            (seq(CertSeq, [], Delta, interpSS(P, Vars, S))) -> 
            write(" non passato\n\n") ; 
            write(" passato\n\n")
        ) ; (
            (E == noVar) ->
...
queryF(31, P, loop) :- 
    P = while(b(true), skip).
    % FAIL       
%! queryF(+QueryNumber, -Program, -Error).
